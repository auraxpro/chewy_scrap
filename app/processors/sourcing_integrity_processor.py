"""
Sourcing Integrity Processor

This module processes product_details records and populates the sourcing_integrity
and sourcing_integrity_reason fields in the processed_products table.

Usage:
    from app.processors.sourcing_integrity_processor import SourcingIntegrityProcessor

    processor = SourcingIntegrityProcessor(db_session)
...
class SourcingIntegrityProcessor:
    """
    Processor for classifying sourcing integrity and updating processed_products table.
    """
    ...
    def __init__(self, db: Session, processor_version: str = "v1.0.0"):
        ...
        self.classifier = SourcingIntegrityClassifier()
    ...
    def process_single(self, product_detail_id: int) -> ProcessedProduct:
        ...
        processed.sourcing_integrity = result.sourcing_integrity.value
        processed.sourcing_integrity_reason = result.reason
    ...
    def process_all(
        ...
        Args:
            ...
            skip_existing: If True, skip records that already have sourcing_integrity set
        ...
        if skip_existing:
            # Only process records without processed_products or without sourcing_integrity
    ...
    def reprocess_category(
        self, sourcing_integrity: str, limit: Optional[int] = None
    ) -> dict:
        """
        Reprocess all records of a specific sourcing integrity category.

        Useful for testing or when classification logic changes.

        Args:
            sourcing_integrity: Sourcing integrity category to reprocess
        ...
        query = self.db.query(ProcessedProduct).filter(
            ProcessedProduct.sourcing_integrity == sourcing_integrity
        )
        ...
        print(
            f"Reprocessing {len(detail_ids)} records with sourcing integrity '{sourcing_integrity}'"
        )
        ...
        if not detail_ids:
            return {
                ...
                "success": 0,
                "failed": 0,
                "message": f"No records found with sourcing integrity '{sourcing_integrity}'",
            }
    ...
    def get_statistics(self) -> dict:
        """
        Get statistics about processed sourcing integrity categories.
        ...
        # Count by sourcing integrity
        results = (
            self.db.query(
                ProcessedProduct.sourcing_integrity,
                func.count(ProcessedProduct.id),
            )
            .group_by(ProcessedProduct.sourcing_integrity)
            .all()
        )
    ...
    def print_statistics(self):
        """Print sourcing integrity statistics in a readable format."""
        stats = self.get_statistics()

        print("\n" + "=" * 70)
        print("SOURCING INTEGRITY STATISTICS")
        print("=" * 70)

        # Categories
        print("\nBy Sourcing Integrity:")
        categories = [
            "Human Grade (organic)",
            "Human Grade",
            "Feed Grade",
...
def process_sourcing_integrity(
    db: Session,
    ...
    """
    Convenience function to process sourcing integrity.

    Args:
        db: Database session
        product_detail_ids: Specific IDs to process (None = all)
        limit: Max records to process
        skip_existing: Skip records with existing sourcing_integrity
...
if __name__ == "__main__":
    # Test/demo mode
    print("Sourcing Integrity Processor - Test Mode")
    print("=" * 70)
    print("This module processes product_details and classifies sourcing integrity.")
    print("\nUsage:")
    print(
        "  from app.processors.sourcing_integrity_processor import SourcingIntegrityProcessor"
    )
    ...
    print("  processor = SourcingIntegrityProcessor(db)")
